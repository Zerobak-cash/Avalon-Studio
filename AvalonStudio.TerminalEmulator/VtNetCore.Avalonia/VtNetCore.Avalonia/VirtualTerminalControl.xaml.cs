    ...
    public class VirtualTerminalControl : TemplatedControl
    {
        ...
        public VirtualTerminalControl()
        {
            ...
            this.GetObservable(TerminalProperty)
                .ObserveOn(AvalonStudio.Compat.AvaloniaDispatcherScheduler.Instance)
                .Subscribe(terminal =>
                {
        ...
        protected override void OnPointerMoved(PointerEventArgs e)
        {
            ...
            if (Connected && (Terminal.UseAllMouseTracking || Terminal.CellMotionMouseTracking) && position.Column >= 0 && position.Row >= 0 && position.Column < Columns && position.Row < Rows)
            {
                ...
                var button =
                    e.KeyModifiers.HasFlag(PointerUpdateKind.LeftButton) ? 0 :
                        e.KeyModifiers.HasFlag(PointerUpdateKind.RightButton) ? 1 :
                            e.KeyModifiers.HasFlag(PointerUpdateKind.MiddleButton) ? 2 :
                            3;  // No button
            ...
            if (e.KeyModifiers.HasFlag(PointerUpdateKind.LeftButton))
            {
        ...
        protected override void OnPointerPressed(PointerPressedEventArgs e)
        {
            ...
            if (!Connected || (Connected && !Terminal.X10SendMouseXYOnButton && !Terminal.X11SendMouseXYOnButton && !Terminal.SgrMouseMode && !Terminal.CellMotionMouseTracking && !Terminal.UseAllMouseTracking))
            {
                ...
                else if (e.KeyModifiers.HasFlag(PointerUpdateKind.RightButton))
                    PasteClipboard();
            }
        ...
        protected override void OnPointerReleased(PointerReleasedEventArgs e)
        {
            ...
            if (!e.KeyModifiers.HasFlag(PointerUpdateKind.LeftButton))
            {
                if (Selecting)
                {
                    ...
                    GetTopLevel(this).Clipboard.SetTextAsync(captured).GetAwaiter().GetResult();
                }
                else
                {
                    TextSelection = null;
            ...
            foreach (var textRow in spans)
            {
                    ...
                    )))
                {
                    ...
                    foreach (var textSpan in textRow.Spans)
                    {
                        ...
                        drawY.Draw(context, new Point(drawX), textLayout);
        ...
        private void AnnotateView(DrawingContext context)
        {
            ...
            for (var i = 0; i < Rows; i++)
            {
                ...
                y.Draw(context, new Point((Bounds.Size.Width - (CharacterWidth / 2 * s.Length))), textLayout);

                s = (i + 1).ToString();

                textLayout = new FormattedText { Text = s.ToString(), Typeface = lineNumberFormat, FontSize = FontSize };
                y.Draw(context, new Point((Bounds.Size.Width - (CharacterWidth / 2 * (s.Length + 3)))), textLayout);
            }
            ...
            0.Draw(context, new Point((Bounds.Size.Width - bigTextLayout.Bounds.Width - 100)), bigTextLayout);
        }
...
//&
        ...
        {
            string text = await GetTopLevel(this).Clipboard.GetTextAsync();
