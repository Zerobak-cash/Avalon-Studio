...
namespace AvalonStudio.Extensibility.Templating
{
    internal static class HelpForTemplateResolution
    {
        ...
        public static IReadOnlyDictionary<ITemplateMatchInfo, string> GetLanguagesForTemplateInfoGroups(IReadOnlyList<ITemplateMatchInfo> templateList, string language, string defaultLanguage)
        {
            IEnumerable<IGrouping<string, ITemplateMatchInfo>> grouped = templateList.GroupBy(x => x.Info.GroupIdentity, x => !string.IsNullOrEmpty(x.Info.GroupIdentity));
            Dictionary<ITemplateMatchInfo, string> templateGroupsLanguages = new Dictionary<ITemplateMatchInfo, string>();
            ...
            foreach (IGrouping<string, ITemplateMatchInfo> grouping in grouped)
            {
                ...
                templateGroupsLanguages[grouping.First()] = string.Join(", ", languageForDisplay);
            }
