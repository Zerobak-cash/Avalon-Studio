...
namespace AvaloniaEdit.AvaloniaMocks
{
    public class MockPlatformRenderInterface : IPlatformRenderInterface
    {
        ...
        public BitmapImpl LoadBitmap(Stream stream)
        {
            return Mock.Of<BitmapImpl>();
        }

        public BitmapImpl LoadBitmap(string fileName)
        {
            return Mock.Of<BitmapImpl>();
        }

        public BitmapImpl LoadBitmap(PixelFormat format, IntPtr data, PixelSize size, Vector dpi, int stride)
        {
            throw new NotImplementedException();
        }
    }
